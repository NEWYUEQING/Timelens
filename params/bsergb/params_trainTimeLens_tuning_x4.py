...
def trainTImeLens_tuning_BSERGB_x4(args):
    paths = ED()
    ...
    paths.save.exp_path = os.path.join(paths.save.save_path, f"{args.model_name}_tuningx4_true" + f"{args.extension}")
    ...
    training_config = ED()
    training_config.dataloader = 'loader_bsergb_timelens'
    ...
    training_config.train_stats.save_im_ep = 16

    validation_config = ED()
    validation_config.dataloader = 'loader_bsergb_timelens'
    validation_config.val_epochs = 27
    validation_config.val_imsave_epochs = 27
    validation_config.weights_save_freq = 3
    validation_config.crop_size = None if hostname == 'server' else 128
